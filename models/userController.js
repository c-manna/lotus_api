...
var userController = {
	userlogin: async (loginData, callback) => {
		try {
			...
			if (!(userData && userData.length)) return callback({ success: false, message: "Login Faiure" });
			const user = userData[0];
			let hash = user.punter_password;
			hash = hash.replace(/^\$2y(.+)$/i, '$2a$1');
			const isMatch = await util.promisify(bcrypt.compare)(loginData.password, hash);
			if (!isMatch) return callback({ success: false, message: "Login Faiure", data: userJson, token: token });
			const payload = { punter_id: user.punter_id, punter_user_name: user.punter_user_name }
			const token = await util.promisify(jwt.sign)(payload, config.JWT.JWT_USER_SECRET, { expiresIn: "120 days" });
			return callback({ success: true, message: "Login success", data: payload, token: token });
		} catch (err) {
			...
			for (var j = 0; j <= eventCompetitionData.length - 1; j++) {
				...
				if (matchCompetitionData.length > 0) {
					for (var i = 0; i <= matchCompetitionData.length - 1; i++) {
						//console.log('matchByCompetetion ID==>',matchCompetitionData[i].event.id);
