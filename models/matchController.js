...
module.exports = {
  ...
  getBookmaker: async (req, res, next) => {
    ...
    let sqlQuery = "SELECT status, details FROM bookmaker WHERE market_id= ? AND status= ?";
    try {
      let dataList = await query(sqlQuery, [marketId, status]);
      return res.send({
        success: true,
        data: (dataList && dataList.length && dataList[0].details) ? JSON.parse(dataList[0].details) : []
      });
    } catch (err) {
      return res.send({
        success: false,
